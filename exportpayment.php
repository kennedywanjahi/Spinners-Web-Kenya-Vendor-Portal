      ...
      if($file_array[1] == "xlsx")
      {
           ...
           foreach($object->getWorksheetIterator() as $worksheet)
           {
                ...
                for($row=2; $row<=$highestRow; $row++)
                {
                     ...
                     $method= mysqli_real_escape_string($connect, $worksheet->getCellByColumnAndRow(2, $row)->getValue());
                     $query = "
                     INSERT INTO payment_methods
                     (PeriodId,  VendorCode, VendorName, Method)
                     VALUES ('".$period_id."', '".$vendorCode."', '".$vendorName."', '".$method."')";
